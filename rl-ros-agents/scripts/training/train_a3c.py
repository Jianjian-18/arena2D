...
def main(log_dir=None, name_results_root_folder="results", use_reward_bound=True, reward_bound=130, gamma=0.95, n_steps=4, time_steps=int(1e5), max_grad_norm=0.1, learn_rate=1e-3):
    ...
    if log_dir is None:
        if args.restart_training:
            ...
            latest_log_dir = os.path.join(name_results_root_folder, sorted(
                os.listdir(name_results_root_folder))[-1])
            logdir = latest_log_dir
        else:
            defaul_log_dir = os.path.join(
                name_results_root_folder, "A3C_" + getTimeStr())
    ...
    if not args.restart_training and (not args.eval):
        ...
        if os.path.exists(path_temp_model+".zip"):
            ...
            print("Can't load the model with the path: {}, please check again!".format(
                path_temp_model))
            envs.close()
            exit(-1)
    else:
    ...
    try:
        ...
        print("KeyboardInterrupt: saved the current model to {}".format(
            path_temp_model))
    finally:
        envs.close()
        exit(0)
...
def parseArgs():
    ...
    parser.add_argument("-r", "--restart_training", action="store_true",
                        help="restart the latest unfinished training")
    parser.add_argument("--eval", action="store_true",
                        help="start the evaluation mode")
    return parser.parse_args()


if __name__ == "__main__":
    main(log_dir=LOGDIR, use_reward_bound=use_reward_bound, reward_bound=REWARD_BOUND, time_steps=TIME_STEPS,
         max_grad_norm=MAX_GRAD_NORM, gamma=GAMMA, learn_rate=LEARNING_RATE, n_steps=N_STEPS)
